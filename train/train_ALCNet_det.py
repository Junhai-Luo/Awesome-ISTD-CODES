...
def build_dataset_components():
    print("[ALC Train] Dataset: utils/acm/data_detlab.py")
    return "detlab", DetlabTxtDetDataset, det_dataset_collate


def parse_args():
    parser = ArgumentParser(description="ALCNet single-frame detector training")
    ...
    parser.add_argument("--image-root", type=str, default=os.environ.get("ALC_IMAGE_ROOT", ""))
    parser.add_argument("--base-size", type=int, default=int(os.environ.get("ALC_BASE_SIZE", 512)))
    parser.add_argument("--batch-size", type=int, default=int(os.environ.get("ALC_BATCH_SIZE", 4)))
    parser.add_argument("--epochs", type=int, default=int(os.environ.get("ALC_EPOCHS", 100)))
    parser.add_argument("--warm-up-epochs", type=int, default=int(os.environ.get("ALC_WARM_UP_EPOCHS", 0)))
    ...
    parser.add_argument("--fuse-mode", type=str, default=os.environ.get("ALC_FUSE_MODE", "AsymBi"), choices=["AsymBi"])
    parser.add_argument("--det-mode", type=str, default=os.environ.get("ALC_DET_MODE", "feature"), choices=["feature", "saliency"])
    parser.add_argument("--blocks-per-layer", type=int, default=int(os.environ.get("ALC_BLOCKS_PER_LAYER", 4)))
    parser.add_argument("--num-classes", type=int, default=int(os.environ.get("ALC_NUM_CLASSES", 1)))
    parser.add_argument("--stride", type=int, default=int(os.environ.get("ALC_STRIDE", 8)))
    parser.add_argument("--num-workers", type=int, default=int(os.environ.get("ALC_NUM_WORKERS", 2)))
    parser.add_argument("--save-period", type=int, default=int(os.environ.get("ALC_SAVE_EVERY", 10)))
    parser.add_argument("--mosaic", type=int, default=int(os.environ.get("ALC_MOSAIC", "1")))
    parser.add_argument("--mosaic-prob", type=float, default=float(os.environ.get("ALC_MOSAIC_PROB", "0.5")))
    parser.add_argument("--mixup", type=int, default=int(os.environ.get("ALC_MIXUP", "1")))
    parser.add_argument("--mixup-prob", type=float, default=float(os.environ.get("ALC_MIXUP_PROB", "0.5")))
    parser.add_argument("--special-aug-ratio", type=float, default=float(os.environ.get("ALC_SPECIAL_AUG_RATIO", "0.7")))

    parser.add_argument("--eval-flag", type=int, default=int(os.environ.get("ALC_EVAL_FLAG", "1")))
    parser.add_argument("--eval-period", type=int, default=int(os.environ.get("ALC_EVAL_PERIOD", "10")))
    parser.add_argument("--eval-json-path", type=str, default=os.environ.get("ALC_EVAL_JSON_PATH", ""))
    parser.add_argument("--eval-image-root", type=str, default=os.environ.get("ALC_EVAL_IMAGE_ROOT", ""))
    parser.add_argument("--eval-confidence", type=float, default=float(os.environ.get("ALC_EVAL_CONFIDENCE", "0.001")))
    parser.add_argument("--eval-nms-iou", type=float, default=float(os.environ.get("ALC_EVAL_NMS_IOU", "0.65")))

    dataset_name = os.environ.get("DATASET_NAME", "dataset")
    network_name = os.environ.get("NETWORK_NAME", "alcnet")
...
class Trainer(object):
    def __init__(self, args):
        ...
        channels = [8, 16, 32, 64]
        model_cls = ALCNetSaliencyDet if args.det_mode == "saliency" else ALCNetDet
        self.net = model_cls(in_channels=3, layers=layer_blocks, channels=channels, fuse_mode=args.fuse_mode, num_classes=args.num_classes)
        print("[ALC Train] Model:", model_cls.__name__, "det_mode=%s" % args.det_mode)
        ...
        if not self.eval_enabled:
            print("[ALC Eval] Disabled because eval_json_path is empty.")
    ...
    def evaluate_map(self, epoch):
        ...
        try:
            ...
            print("[ALC Eval] pycocotools unavailable, skip eval:", str(e))
            return None
        ...
        self.net.eval()
        for image_id in tqdm(image_ids, desc="ALC Eval", leave=False):
...
def main():
    ...
    if not args.train_txt_path or not args.val_txt_path:
        raise ValueError("ALCNet training requires train/val txt paths. Please set TRAIN_ANNOTATION_PATH and VAL_ANNOTATION_PATH.")
