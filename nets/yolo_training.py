...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        elif self.loss_type == 'piou':
            loss = piou(pred, target, xywh=True, PIoU=False, PIoU2=True, Lambda=1.3, eps=1e-7)
...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, strides=[8, 16, 32], loss_type="ciou", use_nwd=True, use_focal=True):
        super().__init__()
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        if self.use_nwd and reg_targets.numel() > 0:
            nwd = wasserstein_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)
            nwd_loss = (1.0 - nwd).mean()
            loss_iou = 0.5 * nwd_loss + 0.5 * loss_iou
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image, cls_preds_per_image, obj_preds_per_image, expanded_strides, x_shifts, y_shifts):
        ...
        pair_wise_ious      = torch.nan_to_num(pair_wise_ious, nan=0.0, posinf=0.0, neginf=0.0).clamp(min=0.0, max=1.0)
        ...
        if self.fp16:
            with torch.cuda.amp.autocast(enabled=False):
                ...
                cls_preds_          = torch.nan_to_num(cls_preds_, nan=0.0, posinf=1.0, neginf=0.0).clamp(min=0.0, max=1.0)
    ...
    def bboxes_iou(self, bboxes_a, bboxes_b, xyxy=True):
        ...
        return area_i / torch.clamp(area_a[:, None] + area_b - area_i, min=1e-16)
...
def wasserstein_loss(pred, target, eps=1e-7, constant=12):
    pred = pred.view(-1, 4)
    target = target.view(-1, 4)
    center1 = pred[:, :2]
    center2 = target[:, :2]
