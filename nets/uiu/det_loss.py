...
class YOLOLoss(nn.Module):
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        return (5.0 * loss_iou + loss_obj + loss_cls) / num_fg
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image, cls_preds_per_image, obj_preds_per_image, expanded_strides, x_shifts, y_shifts):
        ...
        cls_preds_ = cls_preds_.clamp(min=self.eps, max=1.0 - self.eps)
        gt_cls_per_image = F.one_hot(gt_classes.to(torch.int64), self.num_classes).float().unsqueeze(1).repeat(1, num_in_boxes_anchor, 1)
        cls_preds_sqrt = torch.sqrt(cls_preds_).clamp(min=self.eps, max=1.0 - self.eps)
        pair_wise_cls_loss = F.binary_cross_entropy(cls_preds_sqrt.float(), gt_cls_per_image.float(), reduction="none").sum(-1)
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt, center_radius=2.5):
        ...
        b_l = x_centers_per_image - gt_l
        b_r = gt_r - x_centers_per_image
        b_t = y_centers_per_image - gt_t
        b_b = gt_b - y_centers_per_image
        ...
        c_l = x_centers_per_image - gt_l
        c_r = gt_r - x_centers_per_image
        c_t = y_centers_per_image - gt_t
        c_b = gt_b - y_centers_per_image
